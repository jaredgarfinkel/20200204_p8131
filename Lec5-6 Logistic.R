...
data1$Internal.Status1 <- relevel(data1$Internal.Status, "NOSHOW") # switch level order, use NOSHOW as reference (=0)

fit=glm(Internal.Status~Appointment.Lag,family=binomial(link='logit'),data=data1)
summary(fit) 
fit1=glm(Internal.Status1~Appointment.Lag,family=binomial(link='logit'),data=data1)
summary(fit1)
